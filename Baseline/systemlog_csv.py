...
log_pattern = re.compile(r"^(?P<timestamp>.+?) - INFO - CPU Usage: (?P<cpu_usage>\d+\.?\d*)% \| Memory Usage: (?P<memory_usage>\d+\.?\d*)% \| Disk Usage: (?P<disk_usage>\d+\.?\d*)%")
...
def convert_log_to_csv(log_filename, csv_filename):
    with open(log_filename, 'r') as log_file, open(csv_filename, 'w', newline='') as csv_file:
        ...
        # Write CSV header
        csv_writer.writerow(["Timestamp", "CPU Usage (%)", "Memory Usage (%)", "Disk Usage (%)"])
        ...
        for line in log_file:
            ...
            if match:
                ...
                csv_writer.writerow([timestamp, cpu_usage, memory_usage, disk_usage])
